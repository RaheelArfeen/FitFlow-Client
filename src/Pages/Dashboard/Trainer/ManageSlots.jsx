        ...
        >
            ...
            <motion.div variants={containerVariants} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <motion.div variants={itemVariants} className="bg-white rounded-xl shadow-lg p-6 flex items-center justify-between">
                    <div>
                        <h3 className="text-2xl font-bold text-gray-800">{trainerSlots.length}</h3>
                ...
                <motion.div variants={itemVariants} className="bg-white rounded-xl shadow-lg p-6 flex items-center justify-between">
                    <div>
                        <h3 className="text-2xl font-bold text-gray-800">{bookedSlots.length}</h3>
                ...
                <motion.div variants={itemVariants} className="bg-white rounded-xl shadow-lg p-6 flex items-center justify-between">
                    <div>
                        <h3 className="text-2xl font-bold text-gray-800">{availableSlots.length}</h3>
            ...
            <motion.div variants={itemVariants} className="bg-white rounded-xl shadow-lg overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                    <h2 className="text-xl font-bold text-gray-800">Your Training Slots</h2>
                    <Link to="/add-slot">
                        <motion.button
                ...
                {trainerSlots.length > 0 ? (
                                    ...
                                    {trainerSlots.map((slot) => (
                                        ...
                                        >
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div>
                                                    <div className="text-sm font-medium text-gray-900">{slot.slotName}</div>
                                                    <div className="text-sm text-gray-500">{slot.classType || 'Personal Training'}</div>
                                                    {slot.maxParticipants && (
                                                        <div className="text-xs text-gray-400">Max: {slot.maxParticipants} participants</div>
                                                    )}
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center space-x-2">
                                                    ...
                                                    <div>
                                                        <div className="text-sm text-gray-900">{slot.days.join(', ')}</div>
                                            ...
                                            <td className="px-6 py-4">
                                                {slot.bookedMembers && slot.bookedMembers.length > 0 ? (
                                                    <div className="text-sm">
                                                        <div className="font-medium text-gray-900">{slot.bookedMembers[0].name}</div>
                                                        ...
                                                        {slot.bookedMembers[0].package && (
                                                            <div className="text-sm text-gray-500">{slot.bookedMembers[0].package}</div>
                                                        )}
                                                    </div>
                                                ) : (
                    ...
                    <motion.div variants={itemVariants} className="p-12 text-center">
                        ...
                        <h3 className="text-lg font-semibold text-gray-800 mb-2">No Slots Created</h3>
                        <p className="text-gray-600 mb-6">You haven't created any training slots yet.</p>
                        <Link to="/add-slot">
                            <motion.button
                                whileHover={{ scale: 1.05 }}
                                whileTap={{ scale: 0.95 }}
                        ...
                        >
                            <div className="p-6">
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <h3 className="font-semibold text-gray-800 mb-2">Slot Information</h3>
                                        <div className="bg-gray-50 p-3 rounded-lg">
                                            <div className="text-sm text-gray-600">Slot: {selectedSlot.slotName}</div>
                                            <div className="text-sm text-gray-600">Time: {selectedSlot.slotTime}</div>
                                            <div className="text-sm text-gray-600">Day: {selectedSlot.days.join(', ')}</div>
                                            <div className="text-sm text-gray-600">Class: {selectedSlot.classType || 'Personal Training'}</div>
                                        </div>
                                    </div>

                                    {selectedSlot.bookedMembers && selectedSlot.bookedMembers.length > 0 && (
                                        <div>
                                            <h3 className="font-semibold text-gray-800 mb-2">Client Information ({selectedSlot.bookedMembers.length} Participants)</h3>
                                            <div className="bg-gray-50 p-3 rounded-lg space-y-3 max-h-[180px] overflow-y-auto custom-scrollbar">
                                                {selectedSlot.bookedMembers.map((member, index) => (
                                                    <div key={index} className="flex items-center space-x-3 border-b border-gray-200 pb-2 last:border-b-0 last:pb-0">
                                                        ...
                                                        <div>
                                                            ...
                                                            {member.package && <div className="text-xs text-gray-500">Package: {member.package}</div>}
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="bg-gray-50 p-3 rounded-lg">
                                            <div className="text-sm text-gray-600">Booked on: Jan 15, 2024</div>
                                            <div className="text-sm text-gray-600">Payment Status: Completed</div>
                                            <div className="text-sm text-gray-600">Package: Premium Membership - $100</div>
                                            <div className="text-sm text-gray-600">Session Type: {selectedSlot.classType || 'Personal Training'}</div>
                                        </div>
                                    </div>
